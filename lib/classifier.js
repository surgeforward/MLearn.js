...
module.exports = function (algorithmType, properties) {
    ...
    classification.score = function (X, Y) {
        ...
        return classification.predict(X).then(function (predictions) {
            var totalCorrect = _.reduce(predictions, function (totalError, x, key) {
                return totalError += +(x === classification.targets[key]);
            }, 0);

            return totalCorrect / classification.targets.length;
        });
    };
