...
module.exports = function (properties) {
    ...
    model.predict = function (x) {
        var deferModelPredict = Q.defer();

        model.soryByDistanceTo(x).then(function (sortedFeatures) {
            ...
            deferModelPredict.resolve(majorityClass);
        });

        return deferModelPredict.promise;
    };
    ...
    model.soryByDistanceTo = function (x) {
        var deferSort = Q.defer();

        async.sortBy(model.features,
            function (feature, callback) {
                setTimeout(function() {
                    callback(null, model.getDistanceBetween(feature, x));
                });
            },
            function (error, sortedFeatures) {
                if (error) {
                    return deferSort.reject(error);
                }

                deferSort.resolve(sortedFeatures);
            }
        );

        return deferSort.promise;
    };
